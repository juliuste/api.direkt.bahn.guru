...
const reachableForDay = async (date, stationId) => {
	...
	const trainDepartures = departures.filter(d => d.line && d.line.mode === 'train')
...
module.exports = async (req, res, next) => {
	...
	if (!id || id.slice(0, 2) !== '80' || !isLocationCode(id)) return next(err('id must be a german uic station code', 400))
